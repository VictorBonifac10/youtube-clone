 // Dados dos vídeos
    ...
    videos.forEach((videos, i) => {
        ...
        <div class="relative bg-gray-200 rounded-xl overflow-hidden">
            <img src="https://picsum.photos/320/180?random=${i + 1}" class="w-full h-full object-cover">
            <span class="absolute bottom-2 right-2 bg-black bg-opacity-80 text-white text-xs px-1 rounded">${videos.duration}</span>
             ${videos.isLive ? '<span class="absolute top-2 right-2 bg-red-600 text-white text-xs px-2 py-1 rounded uppercase font-semibold">Ao vivo</span>' : ''}
        </div>
        <div class="flex gap-3 mt-3">
            <div>
                <h3 class="font-medium text-sm line-clamp-2">${videos.title}</h3>
                <p class="text-sm text-gray-600 mt-1">${videos.channel}</p>
                <p class="text-sm text-gray-600">${videos.isLive ? `${videos.views} assistindo ${videos.time}` : `${videos.views} visualizações • ${videos.time}`}</p>
            </div>
        </div>
        `
